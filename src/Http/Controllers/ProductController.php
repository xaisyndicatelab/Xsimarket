...
use Xsimarket\Database\Models\Wishlist;
use Xsimarket\Database\Models\Variation;
use Xsimarket\Exceptions\XsimarketException;
use Illuminate\Database\Eloquent\Collection;
use Xsimarket\Http\Requests\ProductCreateRequest;
use Xsimarket\Http\Requests\ProductUpdateRequest;
use Xsimarket\Database\Repositories\ProductRepository;
...
class ProductController extends CoreController
{
    ...
    public function fetchProducts(Request $request)
    {
        ...
        if (in_array('variation_options.digital_files', explode(';', $request->with)) || in_array('digital_files', explode(';', $request->with))) {
            throw new XsimarketException(config('shop.app_notice_domain') . 'ERROR.NOT_AUTHORIZED');
        }
        ...
        if (isset($shop_id)) {
            ...
            foreach ($products as $key => $product) {
                if (!isset($product['type_id'])) {
                    throw new XsimarketException("XSIMARKET_ERROR.WRONG_CSV");
                }
                unset($product['id']);
    ...
    public function importVariationOptions(Request $request)
    {
        ...
        if (count($requestFile)) {
            ...
            throw new XsimarketException(CSV_NOT_FOUND);
        }
    ...
    public function calculateRentalPrice(Request $request)
    {
        ...
        if (!$product->is_rental) {
            throw new XsimarketException(NOT_A_RENTAL_PRODUCT);
        }
        ...
        if ($variation_id) {
            ...
            if (!$isAvailable) {
                throw new XsimarketException(NOT_AVAILABLE_FOR_BOOKING);
            }
        } else {
