...
class CheckoutRepository
{
    ...
    public function verify($request)
    {
        ...
        $shipping_charge = isset($settings['options']['freeShipping']) && (isset($settings['options']['freeShippingAmount']) <= $amount) ? 0 : $this->calculateShippingCharge($request, $amount);
        $tax = $this->calculateTax($request, $shipping_charge, $amount);
        $total = $amount + $tax + $shipping_charge;
        if ($total < $minimumOrderAmount) {
            throw new XsimarketException('Minimum order amount is ' . $minimumOrderAmount);
        }
        return [
            'total_tax'            => $tax,
